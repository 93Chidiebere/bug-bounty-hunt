...
import { runMobileUIAutomation } from './appium-driver.js';
...
export async function runNativeScan(filePath, fileName, onLog, onBugFound) {
  ...
  try {
    ...
    // Now trigger Appium dynamic scan
    await runMobileUIAutomation(filePath, onLog, onBugFound);
    
  } catch (err) {
    onLog(`[ERROR] Failed to process native binary: ${err.message}`);
  }
}
